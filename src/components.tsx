...
export function ShadowRow({ sh, dragHandlers, onChangeName, onChangeValue, onDelete, onDuplicate, checked, onCheck }: { sh: ShadowToken; dragHandlers: DragHandlers; onChangeName: (v: string) => void; onChangeValue: (v: string) => void; onDelete: () => void; onDuplicate: () => void; checked?: boolean; onCheck?: (id: number) => void }) {
  const [open,setOpen]=useState(false), [hov,setHov]=useState(false);
  return <div className="shadow-row"><div draggable onDragStart={()=>dragHandlers.onDragStart(sh.id)} onDragOver={(e)=>dragHandlers.onDragOver(e,sh.id)} onDrop={()=>dragHandlers.onDrop()} onDragEnd={()=>dragHandlers.onDragEnd()} className={`shadow-row__header${checked?" shadow-row--checked":""}${hov?" shadow-row--grab":""}`}>{onCheck !== undefined && <input type="checkbox" checked={!!checked} onChange={()=>onCheck(sh.id)} className="chk" />}<DragHandle onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} /><span className="prefix flex-shrink-0" style={{width:72}}>shadow /</span><input value={sh.name} onChange={e=>onChangeName(e.target.value)} className="inp" style={{width:140,flexShrink:0}} /><input value={sh.value} onChange={e=>onChangeValue(e.target.value)} className="inp inp--mono" style={{flex:1,fontSize:12,minWidth:0}} /><ShadowSwatch value={sh.value} active={open} onClick={()=>setOpen(o=>!o)} /><button onClick={onDuplicate} className="dup-btn">⧉</button><button onClick={onDelete} className="del-btn" style={{fontSize:18,flexShrink:0}}>x</button></div>{open && <ShadowPicker value={sh.value} onChange={onChangeValue} />}</div>;
}
