...
const defaultTextStyles = [
  {id:1, group:"display", name:"2xl",     fontFamily:"Inter", fontSize:"72", fontWeight:"700", lineHeight:"1.1",  letterSpacing:"-1",   paragraphSpacing:"0", textDecoration:"NONE"},
  {id:2, group:"display", name:"xl",      fontFamily:"Inter", fontSize:"60", fontWeight:"700", lineHeight:"1.1",  letterSpacing:"-0.5", paragraphSpacing:"0", textDecoration:"NONE"},
  {id:3, group:"heading", name:"h1",      fontFamily:"Inter", fontSize:"36", fontWeight:"700", lineHeight:"1.25", letterSpacing:"-0.5", paragraphSpacing:"0", textDecoration:"NONE"},
  {id:4, group:"heading", name:"h2",      fontFamily:"Inter", fontSize:"30", fontWeight:"600", lineHeight:"1.3",  letterSpacing:"-0.3", paragraphSpacing:"0", textDecoration:"NONE"},
  {id:5, group:"heading", name:"h3",      fontFamily:"Inter", fontSize:"24", fontWeight:"600", lineHeight:"1.35", letterSpacing:"0",    paragraphSpacing:"0", textDecoration:"NONE"},
  {id:6, group:"heading", name:"h4",      fontFamily:"Inter", fontSize:"20", fontWeight:"600", lineHeight:"1.4",  letterSpacing:"0",    paragraphSpacing:"0", textDecoration:"NONE"},
  {id:7, group:"body",    name:"lg",      fontFamily:"Inter", fontSize:"18", fontWeight:"400", lineHeight:"1.6",  letterSpacing:"0",    paragraphSpacing:"0", textDecoration:"NONE"},
  {id:8, group:"body",    name:"default", fontFamily:"Inter", fontSize:"16", fontWeight:"400", lineHeight:"1.5",  letterSpacing:"0",    paragraphSpacing:"0", textDecoration:"NONE"},
  {id:9, group:"body",    name:"sm",      fontFamily:"Inter", fontSize:"14", fontWeight:"400", lineHeight:"1.5",  letterSpacing:"0",    paragraphSpacing:"0", textDecoration:"NONE"},
  {id:10,group:"label",   name:"lg",      fontFamily:"Inter", fontSize:"14", fontWeight:"500", lineHeight:"1.25", letterSpacing:"0.5",  paragraphSpacing:"0", textDecoration:"NONE"},
  {id:11,group:"label",   name:"default", fontFamily:"Inter", fontSize:"12", fontWeight:"500", lineHeight:"1.25", letterSpacing:"0.5",  paragraphSpacing:"0", textDecoration:"NONE"},
  {id:12,group:"label",   name:"sm",      fontFamily:"Inter", fontSize:"11", fontWeight:"500", lineHeight:"1.25", letterSpacing:"1",    paragraphSpacing:"0", textDecoration:"NONE"},
  {id:13,group:"caption", name:"default", fontFamily:"Inter", fontSize:"12", fontWeight:"400", lineHeight:"1.4",  letterSpacing:"0",    paragraphSpacing:"0", textDecoration:"NONE"},
  {id:14,group:"code",    name:"default", fontFamily:"Fira Code", fontSize:"14", fontWeight:"400", lineHeight:"1.6", letterSpacing:"0",  paragraphSpacing:"0", textDecoration:"NONE"},
];
...
const hdrStyle: any = {fontSize:11,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.07em",color:"#555",padding:"10px 0 6px",display:"flex",alignItems:"center",gap:8};

// ── Shared components ─────────────────────────────────────────────────────────
function AddRowBtn({ onClick, label }: any) {
  return <button onClick={onClick} style={{fontSize:12,padding:"6px 12px",borderRadius:6,border:"1px dashed #333",background:"transparent",color:"#555",cursor:"pointer",marginTop:6,width:"100%",textAlign:"left"}}>{label}</button>;
}
function TabHeader({ title, description, actions }: any) {
  return (
    <div style={{marginBottom:20,paddingBottom:16,borderBottom:"1px solid #1e1e30"}}>
      ...
      {description && <div style={{fontSize:13,color:"#666",marginTop:4}}>{description}</div>}
    </div>
  );
}
function DragHandle({ onMouseEnter, onMouseLeave }: any) {
  return <div title="Drag to reorder" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} style={{cursor:"grab",padding:"0 4px",color:"#444",fontSize:14,userSelect:"none",display:"flex",alignItems:"center",flexShrink:0}}>&#8959;</div>;
}
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:4}}>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        <span style={{fontSize:11,color:"#666",fontFamily:"monospace"}}>{isCustom ? resolved : value.replace("{primitives.","").replace("}","")}</span>
      </div>
      <select value={isCustom?"custom":value} onChange={e=>{const v=e.target.value;if(v==="custom"){setShowCustom(true);onChange(resolved);}else{setShowCustom(false);onChange(v);}}} style={{background:"#1a1a2e",border:"1px solid #333",borderRadius:6,padding:"8px 10px",fontSize:12,color:"#ddd",outline:"none",cursor:"pointer"}}>
        <optgroup label={"-- "+mode+" mode --"}>{opts.map(o=><option key={o.ref} value={o.ref} style={{background:"#1a1a2e"}}>{o.label}</option>)}</optgroup>
      </select>
      {showCustom && <div style={{display:"flex",gap:6,alignItems:"center"}}><input type="color" value={resolved} onChange={e=>onChange(e.target.value)} style={{width:32,height:32,border:"none",background:"none",cursor:"pointer",padding:0}} /><input value={value} onChange={e=>onChange(e.target.value)} placeholder="#000000 or rgba(...)" style={{flex:1,background:"#1a1a2e",border:"1px solid #444",borderRadius:6,padding:"8px 10px",fontSize:12,color:"#fff",outline:"none",fontFamily:"monospace"}} /></div>}
    </div>
  );
}
...
function ShadowSlider({ label, value, min, max, onChange }: any) {
  return <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:11,color:"#666",width:52,flexShrink:0}}>{label}</span><input type="range" min={min} max={max} value={value} onChange={e=>onChange(Number(e.target.value))} style={{flex:1,accentColor:"#4f46e5",cursor:"pointer"}} /><input type="number" value={value} onChange={e=>onChange(Number(e.target.value)||0)} style={inp({width:52,padding:"4px 6px",fontSize:11,textAlign:"center",fontFamily:"monospace"})} /><span style={{fontSize:11,color:"#555",width:16,flexShrink:0}}>px</span></div>;
}
...
function ShadowRow({ sh, dragHandlers, onChangeName, onChangeValue, onDelete }: any) {
  ...
  return <div style={{borderBottom:"1px solid #14141e"}}><div draggable onDragStart={()=>dragHandlers.onDragStart(sh.id)} onDragOver={(e: any)=>dragHandlers.onDragOver(e,sh.id)} onDrop={()=>dragHandlers.onDrop()} onDragEnd={()=>dragHandlers.onDragEnd()} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0 8px 8px",cursor:hov?"grab":"default"}}><DragHandle onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} /><span style={{fontSize:12,color:"#555",flexShrink:0,width:72}}>shadow /</span><input value={sh.name} onChange={e=>onChangeName(e.target.value)} style={inp({width:140,flexShrink:0})} /><input value={sh.value} onChange={e=>onChangeValue(e.target.value)} style={inp({flex:1,fontFamily:"monospace",fontSize:12,minWidth:0})} /><ShadowSwatch value={sh.value} active={open} onClick={()=>setOpen((o: boolean)=>!o)} /><button onClick={onDelete} style={{...delBtn,fontSize:18,flexShrink:0}}>x</button></div>{open && <ShadowPicker value={sh.value} onChange={onChangeValue} />}</div>;
}
...
function DownloadPanel({ enabled, primGroups, primitives, colors, spacing, typography, textStyles, radius, borders, shadows, zindex, breakpoints }: any) {
  ...
  return (
    <div style={{background:"#111118",border:"1px solid #2a2a3e",borderRadius:10,padding:16,minWidth:340,boxShadow:"0 8px 32px rgba(0,0,0,0.5)"}}>
      <div style={{background:"#0a0a14",border:"1px solid #1e1e30",borderRadius:7,padding:"10px 12px",marginBottom:12,fontSize:11,color:"#666",lineHeight:1.7}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"7px 8px",marginBottom:4,borderRadius:6,background:"#0f0f1a",border:"1px solid #1e1e30"}}>
        ...
        <span style={{fontSize:11,color:"#555"}}>{checked.size} / {allFiles.length} selected</span>
      </div>
...
export default function App() {
  ...
  const addTextStyle = (group: string) =>
    setTextStyles(ts => [...ts, {id:uid(),group,name:"new",fontFamily:"Inter",fontSize:"16",fontWeight:"400",lineHeight:"1.5",letterSpacing:"0",paragraphSpacing:"0",textDecoration:"NONE"}]);
  ...
  const handleImport = (e: any) => {
    ...
    reader.onload = (ev: any) => {
      try {
        ...
        if (data.primitives) { const ng: any[]=[],np: any={}; let gi=900; Object.entries(data.primitives).forEach(([k,v]: any)=>{ if(k==="$description")return; if(k==="base"){np.base={};Object.entries(v).forEach(([kk,vv]: any)=>{if(vv?.["$value"])np.base[kk]=vv["$value"]});return;} if(typeof v==="object"){const sh=Object.keys(v).filter((s: string)=>v[s]?.["$value"]);ng.push({id:gi++,key:k,label:k.charAt(0).toUpperCase()+k.slice(1),shades:sh});np[k]={};sh.forEach((s: string)=>{np[k][s]=v[s]["$value"]});}}); if(ng.length){setPrimGroups(ng);setPrimitives(np);} }
        if (data.color) { const cols: any[]=[]; let id=1000; const grps=new Set<string>(); Object.entries(data.color).forEach(([g,tokens]: any)=>{if(typeof tokens!=="object"||tokens["$value"])return;grps.add(g);Object.entries(tokens).forEach(([n,t]: any)=>{if(t["$value"])cols.push({id:id++,group:g,name:n,light:t["$value"],dark:t["$extensions"]?.mode?.dark||t["$value"],description:t["$description"]||""});})}); if(cols.length){setColors(cols);setColorGroups([...grps]);} }
        if (data.spacing) { const sp: any[]=[]; let id=2000; Object.entries(data.spacing).forEach(([n,t]: any)=>{if(t["$value"])sp.push({id:id++,name:n,value:String(t["$value"]).replace("px","")})}); if(sp.length)setSpacing(sp); }
        if (data["border-width"]) { const b: any[]=[]; let id=3500; Object.entries(data["border-width"]).forEach(([n,t]: any)=>{if(t["$value"])b.push({id:id++,name:n,value:String(t["$value"]).replace("px","")})}); if(b.length)setBorders(b); }
        if (data.radius) { const r: any[]=[]; let id=3000; Object.entries(data.radius).forEach(([n,t]: any)=>{if(t["$value"])r.push({id:id++,name:n,value:String(t["$value"]).replace("px","")})}); if(r.length)setRadius(r); }
        // Text styles import
        if (data.heading||data.body||data.display||data.label||data.caption||data.code) {
          ...
          Object.entries(data).forEach(([g,tokens]: any)=>{
            ...
            Object.entries(tokens).forEach(([n,t]: any)=>{
              ...
              const v=t["$value"];
              ts.push({id:id++,group:g,name:n,fontFamily:v.fontFamily||"Inter",fontSize:String(typeof v.fontSize==="object"?v.fontSize.value:v.fontSize||16),fontWeight:String(v.fontWeight||400),lineHeight:String(typeof v.lineHeight==="object"?v.lineHeight.value:v.lineHeight||1.5),letterSpacing:String(typeof v.letterSpacing==="object"?v.letterSpacing.value:v.letterSpacing||0),paragraphSpacing:String(typeof v.paragraphSpacing==="object"?v.paragraphSpacing.value:v.paragraphSpacing||0),textDecoration:v.textDecoration||"NONE"});
            });
          });
          if(ts.length){setTextStyles(ts);setTsGroups(grps);}
        }
    ...
    <div style={{fontFamily:"Inter,system-ui,sans-serif",background:"#0a0a0f",minHeight:"100vh",color:"#fff",display:"flex",flexDirection:"column",width:"100%",boxSizing:"border-box",overflow:"hidden"}}>
      ...
      <div style={{background:"#111118",borderBottom:"1px solid #222230",padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        <div>
          ...
          <div style={{fontSize:12,color:"#555",marginTop:2}}>Builds DTCG JSON for Figma native Variables + Text Styles import</div>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          {importError && <span style={{fontSize:12,color:"#f87171"}}>{importError}</span>}
          <input ref={fileRef} type="file" accept=".json" style={{display:"none"}} onChange={handleImport} />
          <button onClick={()=>fileRef.current.click()} style={{fontSize:13,padding:"8px 14px",borderRadius:6,border:"1px solid #444",background:"#1a1a2e",color:"#ccc",cursor:"pointer"}}>Import JSON</button>
          {showResetConfirm ? (
            <div style={{display:"flex",alignItems:"center",gap:6,background:"#2a1a1a",border:"1px solid #7f1d1d",borderRadius:6,padding:"4px 10px"}}>
              ...
              <button onClick={()=>setShowResetConfirm(false)} style={{fontSize:12,padding:"5px 10px",borderRadius:5,border:"1px solid #444",background:"transparent",color:"#aaa",cursor:"pointer"}}>Cancel</button>
            </div>
          ) : (
          ...
          <button onClick={()=>setShowPreview(v=>!v)} style={{fontSize:13,padding:"8px 14px",borderRadius:6,border:"1px solid #444",background:"#1a1a2e",color:"#ccc",cursor:"pointer"}}>{showPreview?"Hide Preview":"Preview JSON"}</button>
          <button onClick={copy} style={{fontSize:13,padding:"8px 14px",borderRadius:6,border:"1px solid #555",background:"#2a2a3e",color:"#fff",cursor:"pointer"}}>{copied?"Copied!":"Copy JSON"}</button>
          <div style={{position:"relative"}}>
            <button onClick={()=>setShowDl(v=>!v)} style={{fontSize:13,padding:"8px 14px",borderRadius:6,border:"1px solid #444",background:"#1a1a2e",color:"#ccc",cursor:"pointer"}}>Download Files {showDl?"▴":"▾"}</button>
            {showDl && (
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden",width:"100%"}}>
          ...
          {ALL_TABS.map(t => {
            ...
            return (
              <div key={t} style={{display:"flex",alignItems:"center"}}>
                <button onClick={()=>setTab(t)} style={{flex:1,textAlign:"left",padding:"10px 14px",fontSize:13,fontWeight:active?600:400,cursor:"pointer",border:"none",background:active?"#4f46e5":"transparent",color:active?"#fff":enabled?"#777":"#444",transition:"all 0.15s"}}>{t}</button>
        ...
        <div style={{flex:1,overflow:"auto",padding:28,minWidth:0,boxSizing:"border-box"}}>
          ...
          {tab==="Primitives" && (
            <div>
              <TabHeader title="Primitive Colors" description="Raw palette. Click a name to rename. Never apply directly to layers."
                actions={<button onClick={addPrimGroup} style={{fontSize:13,padding:"8px 14px",borderRadius:6,border:"1px solid #444",background:"#1a1a2e",color:"#ccc",cursor:"pointer"}}>+ Add Palette</button>} />
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:12,alignItems:"flex-start"}}>
                    {g.shades.map((shade: string) => (
                      <div key={shade} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                        ...
                        <InlineLabel value={shade} onCommit={(ns: string)=>renameShade(g.key,shade,ns)} style={{fontSize:11,color:"#666",textAlign:"center"}} />
                    ...
                    <div style={{display:"flex",alignItems:"center",height:56}}><button onClick={()=>addShade(g.key)} style={{height:56,width:40,borderRadius:10,border:"1px dashed #333",background:"transparent",color:"#555",cursor:"pointer",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button></div>
                  </div>
                </div>
              ))}
          ...
          {tab==="Colors" && (
            <div>
              <TabHeader title="Semantic Color Tokens" description="Downloads as two files: colors-light.json and colors-dark.json."
                actions={<button onClick={addColorGroup} style={{fontSize:13,padding:"8px 14px",borderRadius:6,border:"1px solid #444",background:"#1a1a2e",color:"#ccc",cursor:"pointer"}}>+ Add Group</button>} />
              {colorGroups.map((g: string) => (
                <div key={g} style={{marginBottom:28}}>
                  ...
                  {groupedColors[g].length===0 && <div style={{fontSize:12,color:"#444",padding:"8px 4px",fontStyle:"italic"}}>No tokens yet.</div>}
                  {groupedColors[g].length > 0 && (
                    <div>
                      <div style={{display:"grid",gridTemplateColumns:"28px 110px 150px 1fr 1fr 32px",gap:10,padding:"4px 0 8px",borderBottom:"1px solid #1e1e30",marginBottom:4}}>
                        {["","Group","Name","Light","Dark",""].map((h,i)=><div key={i} style={{fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
                      </div>
                      {groupedColors[g].map((c: any) => (
                        <DraggableRow key={c.id} id={c.id} dragHandlers={colorDrag}>
                          <div style={{display:"grid",gridTemplateColumns:"110px 150px 1fr 1fr 32px",gap:10,alignItems:"start"}}>
                            ...
                            <div><input value={c.name} onChange={e=>updateColor(c.id,"name",e.target.value)} style={inp({width:"100%",boxSizing:"border-box"})} /><input value={c.description} onChange={e=>updateColor(c.id,"description",e.target.value)} placeholder="Description" style={inp({width:"100%",boxSizing:"border-box",marginTop:6,fontSize:11,color:"#666",padding:"6px 10px",border:"1px solid #222"})} /></div>
          ...
          {tab==="Spacing" && (
            <div>
              <TabHeader title="Spacing Tokens" description="4px base scale. Drag to reorder." />
              <div style={{display:"grid",gridTemplateColumns:"28px 80px 160px 160px 1fr 32px",gap:10,padding:"0 0 8px",borderBottom:"1px solid #1e1e30",marginBottom:4}}>
                {["","Prefix","Name","Value","Visual",""].map((h,i)=><div key={i} style={{fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
              </div>
              {spacing.map((sp: any) => (
                <DraggableRow key={sp.id} id={sp.id} dragHandlers={spacingDrag}>
                  <div style={{display:"grid",gridTemplateColumns:"80px 160px 160px 1fr 32px",gap:10,alignItems:"center"}}>
                    <span style={{fontSize:12,color:"#555"}}>spacing /</span>
                    <input value={sp.name} onChange={e=>updateList(setSpacing,sp.id,"name",e.target.value)} style={inp({width:"100%",boxSizing:"border-box"})} />
                    <div style={{display:"flex",gap:6,alignItems:"center"}}><input value={sp.value} onChange={e=>updateList(setSpacing,sp.id,"value",e.target.value)} style={inp({width:"100%",boxSizing:"border-box",fontFamily:"monospace"})} /><span style={{fontSize:12,color:"#555"}}>px</span></div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{height:14,background:"#4f46e5",borderRadius:3,opacity:0.7,width:Math.min(parseInt(sp.value)||0,220)+"px",minWidth:2}} /><span style={{fontSize:12,color:"#555"}}>{sp.value}px</span></div>
          ...
          {tab==="Typography" && (
            <div>
              <TabHeader title="Typography Tokens" description="Font families, sizes, weights and line heights. For composite text styles, use the Text Styles tab." />
              {[["families","font / family",""],["sizes","font / size","px"],["weights","font / weight",""],["lineHeights","font / line-height",""]].map(([key,label,unit]) => (
                <div key={key} style={{marginBottom:28}}>
                  <div style={{marginBottom:8}}><span style={{fontSize:12,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.07em",color:"#555"}}>{label} — drag to reorder</span></div>
                  <div style={{display:"grid",gridTemplateColumns:"28px 180px 1fr 32px",gap:10,padding:"0 0 6px",borderBottom:"1px solid #1e1e30",marginBottom:4}}>
                    {["","Name","Value",""].map((h,i)=><div key={i} style={{fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
                  </div>
                  {(typography as any)[key].map((item: any) => (
                    <DraggableRow key={item.id} id={item.id} dragHandlers={typoDragMap[key]}>
                      <div style={{display:"grid",gridTemplateColumns:"180px 1fr 32px",gap:10,alignItems:"center"}}>
                        ...
                        <div style={{display:"flex",gap:6,alignItems:"center"}}><input value={item.value} onChange={e=>setTypography((t: any)=>({...t,[key]:t[key].map((i: any)=>i.id===item.id?{...i,value:e.target.value}:i)}))} style={inp({width:"100%",boxSizing:"border-box",fontFamily:key==="families"?"inherit":"monospace"})} />{unit && <span style={{fontSize:12,color:"#555",flexShrink:0}}>{unit}</span>}</div>
          ...
          {tab==="Text Styles" && (
            <div>
              <TabHeader title="Text Styles"
                ...
                actions={<button onClick={addTsGroup} style={{fontSize:13,padding:"8px 14px",borderRadius:6,border:"1px solid #444",background:"#1a1a2e",color:"#ccc",cursor:"pointer"}}>+ Add Group</button>} />
              ...
              {tsGroups.map((g: string) => (
                <div key={g} style={{marginBottom:32}}>
                  ...
                  {/* Column headers */}
                  <div style={{display:"grid",gridTemplateColumns:"28px 80px 100px 180px 54px 60px 64px 64px 110px 32px",gap:8,padding:"4px 0 8px",borderBottom:"1px solid #1e1e30",marginBottom:4,alignItems:"center"}}>
                    {["","Preview","Name","Font Family","Size","Weight","L.Height","L.Space","Decoration",""].map((h,i)=><div key={i} style={{fontSize:10,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
                  </div>

                  {groupedTextStyles[g] && groupedTextStyles[g].map((s: any) => (
                    <DraggableRow key={s.id} id={s.id} dragHandlers={textStylesDrag}>
                      <div style={{display:"grid",gridTemplateColumns:"80px 100px 180px 54px 60px 64px 64px 110px 32px",gap:8,alignItems:"center"}}>

                        {/* Live preview */}
                        <div style={{height:36,display:"flex",alignItems:"center",overflow:"hidden",paddingLeft:2}}>
                          <span style={{fontFamily:s.fontFamily,fontSize:Math.min(parseFloat(s.fontSize)||16,22)+"px",fontWeight:s.fontWeight,lineHeight:s.lineHeight,letterSpacing:(parseFloat(s.letterSpacing)||0)+"px",color:"#fff",whiteSpace:"nowrap",opacity:0.9}}>Ag</span>
                        </div>
                        ...
                        {/* Font family */}
                        <input value={s.fontFamily} onChange={e=>updateTextStyle(s.id,"fontFamily",e.target.value)} title={s.fontFamily} style={inp({width:"100%",boxSizing:"border-box",fontFamily:s.fontFamily,fontSize:11})} />
              ...
              {tsGroups.length === 0 && <div style={{fontSize:13,color:"#444",fontStyle:"italic",padding:"32px 0",textAlign:"center"}}>No groups yet. Click "+ Add Group" to start.</div>}
            </div>
          )}
          ...
          {tab==="Radius" && (
            <div>
              <TabHeader title="Border Radius Tokens" description="Drag to reorder." />
              <div style={{display:"flex",flexWrap:"wrap",gap:20,paddingLeft:8}}>
                {radius.map((r: any) => (
                  <div key={r.id} draggable onDragStart={()=>radiusDrag.onDragStart(r.id)} onDragOver={(e: any)=>radiusDrag.onDragOver(e,r.id)} onDrop={()=>radiusDrag.onDrop()} onDragEnd={()=>radiusDrag.onDragEnd()} style={{background:"#111118",border:"1px solid #222230",borderRadius:12,padding:"20px 16px",display:"flex",flexDirection:"column",alignItems:"center",gap:14,width:120,boxSizing:"border-box",cursor:"grab"}}>
                    ...
                    <div style={{display:"flex",gap:6,alignItems:"center",width:"100%"}}><input value={r.value} onChange={e=>updateList(setRadius,r.id,"value",e.target.value)} style={inp({flex:1,width:0,textAlign:"center",fontFamily:"monospace",padding:"7px 8px"})} /><span style={{fontSize:12,color:"#555",flexShrink:0}}>px</span></div>
          ...
          {tab==="Border" && (
            <div>
              <TabHeader title="Border Width Tokens" description="Drag to reorder." />
              <div style={{display:"grid",gridTemplateColumns:"28px 80px 160px 160px 1fr 32px",gap:10,padding:"0 0 8px",borderBottom:"1px solid #1e1e30",marginBottom:4}}>
                {["","Prefix","Name","Value","Visual",""].map((h,i)=><div key={i} style={{fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
              </div>
              {borders.map((b: any) => (
                <DraggableRow key={b.id} id={b.id} dragHandlers={borderDrag}>
                  <div style={{display:"grid",gridTemplateColumns:"80px 160px 160px 1fr 32px",gap:10,alignItems:"center"}}>
                    <span style={{fontSize:12,color:"#555"}}>border /</span>
                    <input value={b.name} onChange={e=>updateList(setBorders,b.id,"name",e.target.value)} style={inp({width:"100%",boxSizing:"border-box"})} />
                    <div style={{display:"flex",gap:6,alignItems:"center"}}><input value={b.value} onChange={e=>updateList(setBorders,b.id,"value",e.target.value)} style={inp({width:"100%",boxSizing:"border-box",fontFamily:"monospace"})} /><span style={{fontSize:12,color:"#555"}}>px</span></div>
          ...
          {tab==="Shadows" && (
            <div>
              <TabHeader title="Shadow Tokens" description="Drag to reorder. Click the swatch to open the shadow picker." />
              <div style={{display:"grid",gridTemplateColumns:"38px 72px 140px 1fr 80px 32px",gap:10,padding:"0 0 8px 8px",borderBottom:"1px solid #1e1e30",marginBottom:4}}>
                {["","Prefix","Name","Value","Preview",""].map((h,i)=><div key={i} style={{fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
              </div>
              {shadows.map((sh: any) => (
          ...
          {tab==="Z-Index" && (
            <div>
              <TabHeader title="Z-Index Tokens" description="Stacking order reference. Drag to reorder." />
              <div style={{display:"grid",gridTemplateColumns:"28px 100px 180px 180px 1fr 32px",gap:10,padding:"0 0 8px",borderBottom:"1px solid #1e1e30",marginBottom:4}}>
                {["","Prefix","Name","Value","",""].map((h,i)=><div key={i} style={{fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
              </div>
              {zindex.map((z: any) => (
                <DraggableRow key={z.id} id={z.id} dragHandlers={zDrag}>
                  <div style={{display:"grid",gridTemplateColumns:"100px 180px 180px 1fr 32px",gap:10,alignItems:"center"}}>
                    <span style={{fontSize:12,color:"#555"}}>z-index /</span>
          ...
          {tab==="Breakpoints" && (
            <div>
              <TabHeader title="Breakpoint Tokens" description="Min-width based. Drag to reorder." />
              <div style={{display:"grid",gridTemplateColumns:"28px 100px 140px 140px 140px 1fr 32px",gap:10,padding:"0 0 8px",borderBottom:"1px solid #1e1e30",marginBottom:4}}>
                {["","Prefix","Name","Min (px)","Max (px)","Range",""].map((h,i)=><div key={i} style={{fontSize:11,color:"#555",fontWeight:600,textTransform:"uppercase"}}>{h}</div>)}
              </div>
              {breakpoints.map((b: any) => (
                <DraggableRow key={b.id} id={b.id} dragHandlers={breakpointDrag}>
                  <div style={{display:"grid",gridTemplateColumns:"100px 140px 140px 140px 1fr 32px",gap:10,alignItems:"center"}}>
                    <span style={{fontSize:12,color:"#555"}}>breakpoint /</span>
                    ...
                    <div style={{display:"flex",gap:6,alignItems:"center"}}><input value={b.value} onChange={e=>updateList(setBreakpoints,b.id,"value",e.target.value)} style={inp({width:"100%",boxSizing:"border-box",fontFamily:"monospace"})} /><span style={{fontSize:12,color:"#555",flexShrink:0}}>px</span></div>
                    <div style={{display:"flex",gap:6,alignItems:"center"}}><input value={b.max} onChange={e=>updateList(setBreakpoints,b.id,"max",e.target.value)} placeholder="none" style={inp({width:"100%",boxSizing:"border-box",fontFamily:"monospace"})} /><span style={{fontSize:12,color:"#555",flexShrink:0}}>px</span></div>
                    <div style={{fontSize:12,color:"#555",fontFamily:"monospace"}}>{bpRange(b)}</div>
        ...
        {showPreview && (
          <div style={{width:420,background:"#0d0d18",borderLeft:"1px solid #222230",display:"flex",flexDirection:"column",flexShrink:0}}>
            <div style={{padding:"12px 16px",borderBottom:"1px solid #222230",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <span style={{fontSize:11,color:"#555"}}>Figma native format</span>
            </div>
